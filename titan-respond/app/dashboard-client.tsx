...
async function addLead(e:any){
 ...
 <section className="grid"><div className="panel"><div className="panelHead"><div><span className="eyebrow">CRM</span><h3>Lead pipeline</h3></div><button onClick={()=>load().catch(e=>setError(e.message))}>Refresh</button></div><div className="table">{leads.length===0&&<p className="hint">No leads yet.</p>}{leads.map(l=><div className="row" key={l.id}><div><Link href={`/leads/${l.id}`}>

  <strong>{l.name}</strong>

</Link><small>{l.service||'General enquiry'} · {l.source}</small></div><select className={`status ${l.status}`} value={l.status} onChange={e=>status(l.id,e.target.value)}>{statuses.map(s=><option key={s} value={s}>{s.replaceAll('_',' ')}</option>)}</select><div><strong>{l.estimatedValue!=null?`£${l.estimatedValue.toLocaleString()}`:'—'}</strong><small>{l.appointmentAt?new Date(l.appointmentAt).toLocaleString('en-GB'):'No booking yet'}</small></div><button disabled={busy===l.id||l.status==='do_not_contact'} onClick={()=>call(l.id)}>{busy===l.id?'Queued…':'AI call'}</button></div>)}</div></div>
